...
const ChevronRight = ({open}) => (
  <svg className={`chev ${open ? "open":""}`} width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export default function Sidebar(){
  ...
  const toggle = idx => setFolders(prev => prev.map((f,i) => i===idx ? {...f, open: !f.open} : f));
    ...
    <aside className="sidebar-root">
      <div className="sidebar-panel">
        <div className="sidebar-header">
          <h3>библиотека</h3>
          <button className="btn create" onClick={createFolder}>создать папку</button>
        </div>
          ...
          {folders.map((f,i) => (
            <div key={i} className="folder">
              ...
              <div className={`files ${f.open ? "open":""}`}>
                {f.files.length === 0 ? <div className="empty">пусто</div> : f.files.map((file,idx) => (
                  <div key={idx} className="file-row">{file}</div>
                ))}
              </div>
            </div>
          ))}
